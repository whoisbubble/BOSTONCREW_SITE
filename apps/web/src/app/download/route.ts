...
export async function GET() {
  ...
  const archivePath = isAbsolute(configuredPath) ? resolve(configuredPath) : resolve(projectRoot, configuredPath);
  ...
  try {
    ...
    if (!archiveStat.isFile()) {
      return new Response('Archive path is not a file.', { status: 404 });
    }
    ...
    return new Response(`Archive not found. Put ${archiveName} in the project root or set DOWNLOAD_ARCHIVE_NAME.`, {
      status: 404,
    });
  }
}
